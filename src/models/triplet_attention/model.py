...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, att_type=None):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, use_triplet_attention=att_type == 'TripletAttention'))
...
def triplet_attention(n_classes):
    net = ResidualNet("ImageNet", 50, n_classes, TripletAttention)
    return net


if __name__ == '__main__':
    model = triplet_attention(100)
    print(model(torch.zeros(32, 3, 224, 224)).size())